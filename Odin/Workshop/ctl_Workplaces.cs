...
{
    public delegate void SaveWorkplacesEventHandler(object sender);

    public partial class ctl_Workplaces : UserControl
    {
        public ctl_Workplaces()
        {
            InitializeComponent();
        }
        ...
        public ctl_Workplace ctl_WP = null;

        public int ControlWidth = 299;
        ...
        public void FillWorktables(int _roomid)
        {
            ...
            //Clear old controls
            RemoveControls(this, typeof(ctl_Workplace));

            //Fill new

            SqlConnection sqlConn = new SqlConnection(sConnStr);
            SqlCommand sqlComm = new SqlCommand("sp_SelectWorktables", sqlConn);
            ...
            if (reader.HasRows == true)
            {
                ...
                while (reader.Read())
                {
                    ctl_WP = new ctl_Workplace();

                    ctl_WP.LaunchID = Convert.ToInt32(reader["launchid"]);
                    ctl_WP.Workplace = reader["name"].ToString();
                    ctl_WP.WorkPlaceID = Convert.ToInt32(reader["id"]);
        ...
        private void btn_Save_Click(object sender, EventArgs e)
        {
            foreach (var ctl in panel.Controls)
            {
                ...
                {
                    ctl_Workplace ctlWP = ctl as ctl_Workplace;
                    BLL.SaveWorkplace(ctlWP.WorkPlaceID, ctlWP.LaunchID);
                }
            }

            SyncronizeData("user/login", false);
            
            //globClass.ShowMessage("Saving result", "", "Saving: success!");
        }


        private void cmb_Common1_SelectedValueChanged(object sender)
        {
            cmb_Rooms1.PlaceId = cmb_Common1.SelectedValue;
        }

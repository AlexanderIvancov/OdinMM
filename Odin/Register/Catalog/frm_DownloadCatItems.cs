    ...
    public partial class frm_DownloadCatItems : KryptonForm
    {
        ...
        private void btn_Download_Click(object sender, EventArgs e)
        {
            ...
            if (openfiledialog1.ShowDialog() == DialogResult.OK)
            {
                ...
                int totalRows = 1;
                while (glob_Class.NES(myExcel.GetValue("C" + totalRows.ToString())).Trim() != "")
                {
                    totalRows++;
                }
                ...
                {
                    pb_Progress.Value = 0;
                    pb_Progress.Maximum = Math.Max(totalRows - 1, 1);
                });
                ...
                while (k <= totalRows)
                {
                    try
                    {
                        ...
                        if (glob_Class.NES(myExcel.GetValue("A" + k)).Trim() == "" && valC.StartsWith("Да "))
                        {
                            ...
                            DateTime.TryParse(glob_Class.NES(myExcel.GetValue("M" + k)).ToString().Trim(), out dt);
                            string validTill = (dt.ToShortDateString() == "01.01.0001" ? "" : dt.ToShortDateString());
                            ...
                            dr["comments"] = supplierComment; 
                            dr["supplier"] = "ELSO GRUPE UAB";
                            dr["validtill"] = validTill;

    ...
    public partial class frm_BatchProjects : BaseForm
    {
        ...
        public void SetCellsColor()
        {
            foreach (DataGridViewRow row in this.gv_List.Rows)
            {
                ...
                if ((string.IsNullOrWhiteSpace(row.Cells["cn_coconfdate"].Value?.ToString())
                        ? DateTime.Now
                        : DateTime.Parse(row.Cells["cn_coconfdate"].Value.ToString()))
                    < DateTime.Now.AddDays(-21))
                    row.Cells["cn_coconfdate"].Style.BackColor = Color.Crimson;

...
{
    public partial class frm_PurDeliveryIndicator : KryptonForm
    {
        public frm_PurDeliveryIndicator()
        {
            InitializeComponent();
            ED = new ExportData(this.gv_List, "DeliveryIndicator.xls", this.Name);
        }
        ...
        {
            data = PO_BLL.getPurchaseDeliveryIndicators(txt_From.Text.Trim() == "" ? System.DateTime.Now.AddMonths(-6).ToShortDateString() : txt_From.Text.Trim(),
        ...
        {
            int _counttmp = 0;
            int _count = 0;// gv_List.RowCount;
            //var query = data.AsEnumerable();

            //query = query.Where(row => row.Field<int>("diffdays") <= 4);
            //try
            //{
            ...
            {
                if (Convert.ToInt32(row.Cells["cn_diffdays"].Value) <= 4)
                    _counttmp++;
                _count++;
            }
